---
title: 核心概念
description: 掌握 vibe-sync 架构 - 学习同步模式（safe vs prune）、配置层级（project vs user）、manifest 系统、原子操作，以及如何安全地保留环境变量。
---

# 核心概念

了解驱动 vibe-sync 的基本概念。

## vibe-sync 如何工作

vibe-sync 遵循简单但强大的工作流程：

```mermaid
flowchart TD
    A[选择源工具<br/>您的配置源] --> B[vibe-sync 读取配置<br/>Skills, MCP, Agents, Commands]
    B --> C[与目标比较<br/>计算差异]
    C --> D[显示计划<br/>将要更改的内容]
    D --> E{您的批准}
...
```bash
vibe-sync init --user    # 创建用户配置
vibe-sync sync --user    # 同步用户配置
```

## 同步模式

vibe-sync 提供两种同步模式来处理不同场景：
...
vibe-sync 使用 manifest 文件（`.vibe-sync-cache/manifest.json`）来跟踪同步状态。
