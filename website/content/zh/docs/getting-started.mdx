---
title: 快速开始
description: 2 分钟安装 vsync。学习如何初始化配置、同步 AI 氛围编程工具（Claude Code、Cursor、OpenCode、Codex）以及处理常见设置场景。
---
...
vsync 可以通过 `npx` 直接使用无需安装，也可以全局安装以便使用。
...
```bash
vsync init  # 选择 Cursor 作为 SOURCE（参考标准）
...
如果您看到"找不到配置文件"，请先运行 `vsync init` 创建配置。
...
- 运行 `vsync <command> --help` 获取特定命令帮助
- 查看 [FAQ](../faq) 了解常见问题
