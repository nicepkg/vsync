---
title: FAQ
description: Frequently asked questions about vibe-sync. Get answers on choosing source tools, config overwriting, monorepo support, environment variables, migration, troubleshooting, and performance.
---

# Frequently Asked Questions

Common questions and answers about using vibe-sync.
...
### Will vibe-sync overwrite my existing configs?
...
Changes in target tools will be **overwritten** on the next sync. vibe-sync uses a unidirectional sync model:
...
**Yes!** Each project can have its own `.vibe-sync.json`. User-level configs (`~/.vibe-sync.json`) work globally across all projects.
...
├── packages/
│   ├── frontend/
│   │   └── .vibe-sync.json  # Frontend-specific config
│   └── backend/
│       └── .vibe-sync.json  # Backend-specific config
└── .vibe-sync.json          # Monorepo root config
```

### Is it safe to commit `.vibe-sync.json` to git?
...
No, vibe-sync is **unidirectional** (source → targets). This is intentional to maintain a clear source of truth.

To switch directions:
1. Run `vibe-sync init`
...
Yes! By default, `vibe-sync sync` shows a plan and asks for confirmation:
...
vibe-sync automatically converts between these formats.

### Will vibe-sync expose my secrets?
...
Cursor Rules (`.cursorrules` files) are currently **not supported**. vibe-sync focuses on:
- Skills
- MCP servers
- Agents
- Commands
...
**Solution**: Run `vibe-sync init` to create configuration.
...
2. Run `vibe-sync init` and select that tool
3. Run `vibe-sync sync` to populate targets
...
vibe-sync doesn't directly import from other sync tools, but you can:

1. Manually copy configs to one of the supported tools
2. Use that tool as source
3. Initialize vibe-sync
...
### Does vibe-sync cache anything?

Yes! The manifest (`.vibe-sync-cache/manifest.json`) acts as a cache:
...
2. **Commit manifest**: Include `.vibe-sync-cache/` in git
...
- **GitHub Issues**: [Ask a question](https://github.com/nicepkg/vibe-sync/issues/new)
- **Documentation**: Browse other docs sections
- **CLI Help**: Run `vibe-sync --help`
