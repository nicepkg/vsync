...
export async function ensureConfig(
  ...
  try {
    const config = await loadSyncConfig(projectDir, isUserLevel);
    return config;
  } catch (error) {
    ...
    if (
      ...
      const { getToolChoices } = await import("../adapters/registry.js");

      // Detect existing tools
      const detectSpinner = ora("Detecting installed AI coding tools...").start();
      ...
      const toolsAnswer = await inquirer.prompt<{ tools: ToolName[] }>([
        {
          type: "checkbox",
          name: "tools",
          message: "Which tools would you like to sync?",
      ...
      const saveSpinner = ora("Creating configuration file...").start();
      await saveInitConfig(config, projectDir);
      saveSpinner.succeed("Configuration created successfully");
